...
export async function fetchLeadsPage({ fields, fromDate, toDate, paginationId, paginationKey }) {
  const body = {
    page_size: 100,
    fields,
    filter: {
  ...
  if (paginationId) {
    const key = paginationKey || "pagination_id";
    body[key] = paginationId;
    body.pagination_id = paginationId;
    body.paginationId = paginationId;
  }
 
  let methodUsed = "GET";
  let result = await merittoRequest("GET", "/lead/v1/list", body);
 
  if (looksLikeNotFound(result)) {
    methodUsed = "POST";
    result = await merittoRequest("POST", "/lead/v1/list", body);
  }
